...
/*   Updated: 2025/09/07 16:58:42 by jegerman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	psr_optk_check(t_tok *token, int pos)
{
	...
	if (token->type == TOK_WORD)
		return (0);
	tok_chr = (char *[]){"|", "<", "<<", ">", ">>", 0};
	if (token[1].type == TOK_EOL || (pos == 0 && token->type == TOK_PIPE))
	{
...
int	psr_parse_line(char *line, t_core *core)
{
	...
	// - Now we have to copy token into the project structures so we can start
	// executing commands
